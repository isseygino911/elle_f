  ...
  async function handleSubmit(event) {
    ...
    try {
      ...
      for (const [index, file] of files.entries()) {
        setProgress(`${t('submissions.uploading')} ${index + 1}/${files.length + (recording ? 1 : 0)}`)
        ...
        {assignment.accepts_recording && (
          <Field>
            ...
            {recordingSupported ? (
              ...
              <Alert>
                <AlertDescription>{t('submissions.recordingUnsupportedPartial')}</AlertDescription>
              </Alert>
            )}
            {recording && (
              <p className="text-muted-foreground text-sm">
                {t('submissions.recordingAttached')} ({recording.file.type ||
                  RECORDING_CONTENT_TYPE})
              </p>
            )}
          </Field>
